...
__export(schema_exports, {
  ...
  insertEmailSignupSchema: () => insertEmailSignupSchema,
...
var DatabaseStorage = class {
  ...
  async createEmailSignup(insertEmailSignup) {
    const [emailSignup] = await db.insert(emailSignups).values(insertEmailSignup).returning();
    return emailSignup;
  }
  async getEmailSignupByEmail(email) {
    const [emailSignup] = await db.select().from(emailSignups).where(eq(emailSignups.email, email));
    return emailSignup || void 0;
  }
...
async function registerRoutes(app2) {
  ...
  app2.get("/download/source", (req, res) => {
